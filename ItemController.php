...
class ItemController extends Controller
{
	...
	public function fileupload(Request $request)
	{
     ...
     if($request->hasFile('file'))
	 {
       ...
       if(in_array(strtolower($extension), $validextensions))
	   {
		 ...
		 //$fileName = $this->item_slug(Carbon::now()->toDayDateTimeString()).rand(11111, 99999) .'.' . $extension;

         // Uploading file to given path
         //$request->file('file')->move($destinationPath, $fileName);

		       $image = $request->file('file');
		       $img_name = $this->item_slug(Carbon::now()->toDayDateTimeString()).rand(11111, 99999) .'.' . $extension;
	...
	public function view_favorite_item($itemid,$favorite,$liked)
	{
	   ...
	   {
	     return redirect()->back()->with('error', 'Sorry Item already added to favorites');
	   }
	...
	public function update_items(Request $request)
	{
	   ...
	   $item_slug = strtolower($this->item_slug($item_name));
	   $item_token = $request->input('item_token');
	   ...
	   $seller_money_back_days = 0;
        ...
        if($request->input('item_seo_title') != "")
        {
            $item_seo_title = $request->input('item_seo_title');
        }
        else
        {
            $item_seo_title = "";
	  ...
	  $rules = array(
		...
		else
		{
		 ...
		 $updated_item = date('Y-m-d H:i:s');
		    ...
		    $data = array('item_name' => $item_name, 'item_desc' => $item_desc, 'item_thumbnail' => $item_thumbnail, 'item_preview' => $item_preview, 'item_file' => $item_file, 'file_type' => $file_type, 'item_file_link' => $item_file_link, 'item_category' =>$cat_id, 'item_category_parent' => $parent_category_id, 'item_category_type' => $cat_name, 'item_type' => $item_type, 'regular_price' => $regular_price, 'sale_price' => $sale_price, 'extended_price' => $extended_price, 'demo_url' => $demo_url, 'item_tags' => $item_tags, 'item_status' => $item_status, 'item_shortdesc' => $item_shortdesc, 'free_download' => $free_download, 'item_version' => $item_version, 'item_slug' => strtolower($item_slug), 'video_url' => $video_url, 'future_update' => $future_update, 'item_support' => $item_support, 'updated_item' => $updated_item, 'item_flash_request' => $item_flash_request, 'video_preview_type' => $video_preview_type, 'video_file' => $video_file, 'item_type_cat_id' => $item_category, 'seller_refund_term' => $seller_refund_term, 'seller_money_back' => $seller_money_back, 'seller_money_back_days' => $seller_money_back_days, 'item_allow_seo' => $item_allow_seo, 'item_seo_keyword' => $item_seo_keyword, 'item_seo_desc' => $item_seo_desc, 'item_seo_title' => $item_seo_title, 'audio_file' => $audio_file);
		 ...
		 $item_slug = strtolower($itemdata['data']->item_slug);
		 $item_url = URL::to('/item').'/'.strtolower($item_slug).'/'.$item_id;
	...
	public function save_items(Request $request)
	{
	   ...
	   $user_id = $request->input('user_id');
        if(empty($request->input('item_token'))) {
	  ...
	  else
	  {
		...
		else
		{
		  ...
		  $data = array('user_id' => $user_id, 'item_token' => $item_token, 'item_name' => $item_name, 'item_desc' => $item_desc, 'item_thumbnail' => $item_thumbnail, 'item_preview' => $item_preview, 'item_file' => $item_file, 'file_type' => $file_type, 'item_file_link' => $item_file_link, 'item_category' =>$cat_id, 'item_category_parent' => $parent_category_id,  'item_category_type' => $cat_name, 'item_type' => $item_type, 'regular_price' => $regular_price, 'sale_price' => $sale_price, 'extended_price' => $extended_price, 'demo_url' => $demo_url, 'item_tags' => $item_tags, 'item_status' => $item_status, 'item_shortdesc' => $item_shortdesc, 'free_download' => $free_download, 'item_version' => $item_version, 'item_slug' => strtolower($item_slug), 'video_url' => $video_url, 'future_update' => $future_update, 'item_support' => $item_support, 'created_item' => $created_item, 'updated_item' => $updated_item, 'item_flash_request' => $item_flash_request, 'video_preview_type' => $video_preview_type, 'video_file' => $video_file, 'item_type_cat_id' => $item_category, 'seller_refund_term' => $seller_refund_term, 'seller_money_back' => $seller_money_back, 'seller_money_back_days' => $seller_money_back_days, 'item_allow_seo' => $item_allow_seo, 'item_seo_keyword' => $item_seo_keyword, 'item_seo_desc' => $item_seo_desc, 'item_seo_title' => $item_seo_title, 'audio_file' => $audio_file);
	  ...
	  /* subscription code */
	   ...
	   if($getcount == 0)
	   {
          ...
          else if($payment_method == 'ipay')
          {
              ...
              {
                  $convert = Currency::convert($site_currency,'KES',$final_amount);
       ...
       return view('order-confirm')->with($record);*/
		  ...
		  else if($payment_method == 'iyzico')
		  {
			   ...
			   $convertedAmount = $convert['convertedAmount'];
			   $price_amount = number_format((float)$convertedAmount, 2, '.', '');
			   }
			   else
			   {
		  ...
		  else if($payment_method == 'stripe')
		  {
              ...
              $token = $_POST['stripeToken'];
				...
				$customer = \Stripe\Customer::create(array(
					'email' => $order_email,
					'source'  => $token,
                    'name' => $_POST['stripe_first_name'],
                    'shipping' => [
                        'name' => $_POST['stripe_first_name'],
                        'address' => [
                            'line1' => $_POST['stripe_address_line1'],
                            'city' => $_POST['stripe_address_city'],
                            'country' => $order_country,
                            'postal_code' => $_POST['stripe_address_zip']
                        ]
                    ]
				));
	...
	if($x_receipt_link_url == 'subscription')
	{
	   if($done_approval_status == "Y")
	   {
		...
		$user_id = Auth::user()->id;
		$checkoutdata = array('user_subscr_type' => $user_subscr_type, 'user_subscr_date' => $subscr_date, 'user_subscr_item_level' => $user_subscr_item_level, 'user_subscr_item' => $user_subscr_item, 'user_subscr_download_item' => $user_subscr_download_item, 'user_subscr_space_level' => $user_subscr_space_level, 'user_subscr_space' => $user_subscr_space, 'user_subscr_space_type' => $user_subscr_space_type);
